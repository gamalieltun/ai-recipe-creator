...
const RecipeDisplay: React.FC<RecipeDisplayProps> = ({ recipe }) => {
    ...
    if (recipe.error) {
        return (
             <div className="w-full max-w-4xl mx-auto mt-10 bg-red-100 border-l-4 border-red-500 text-red-700 p-6 rounded-r-lg shadow-md" role="alert">
                <p className="font-bold">{t('recipe.error.oops')}</p>
                <p>{recipe.error}</p>
            </div>
        )
    }
    ...
    const EssentialIcon = () => (
        <span className="ml-2 text-green-600" title={t('recipe.essential')}>
    ...
    const OptionalIcon = () => (
        <span className="ml-2 text-gray-400" title={t('recipe.optional')}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div className="w-full max-w-4xl mx-auto mt-10 bg-white p-8 rounded-xl shadow-2xl animate-fade-in-up">
            ...
            <div className="flex flex-wrap gap-4 justify-center text-center mb-8 border-t border-b border-gray-200 py-4">
                <div className="flex-1 min-w-[120px]">
                    <p className="text-sm font-medium text-gray-500">{t('recipe.prepTime')}</p>
                    <p className="text-lg font-bold text-indigo-600">{recipe.prepTime}</p>
                </div>
                <div className="flex-1 min-w-[120px]">
                    <p className="text-sm font-medium text-gray-500">{t('recipe.cookTime')}</p>
                    <p className="text-lg font-bold text-indigo-600">{recipe.cookTime}</p>
                </div>
                <div className="flex-1 min-w-[120px]">
                    <p className="text-sm font-medium text-gray-500">{t('recipe.servings')}</p>
            ...
            <div className="grid md:grid-cols-3 gap-8">
                <div className="md:col-span-1">
                    <h3 className="text-2xl font-bold text-gray-700 mb-4 border-b-2 border-indigo-500 pb-2">{t('recipe.ingredients')}</h3>
                ...
                <div className="md:col-span-2">
                    <h3 className="text-2xl font-bold text-gray-700 mb-4 border-b-2 border-green-500 pb-2">{t('recipe.instructions')}</h3>
